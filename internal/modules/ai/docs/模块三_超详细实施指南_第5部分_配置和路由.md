# 模块三 AI 微服务/小工具 - 超详细实施指南（第5部分：配置和路由）

## 第五部分：配置和路由

这部分实现系统配置和HTTP路由注册，负责：
- 添加微服务配置结构
- 修改配置文件
- 注册HTTP和WebSocket路由
- 创建WebSocket认证中间件
- 初始化所有依赖

---

## 5.1 配置文件修改

### 5.1.1 修改 config.go

#### 文件路径
```
internal/config/config.go
```

#### 修改说明

**在现有的 `AIConfig` 结构体中添加 `Microservice` 字段**

```go
// 找到 AIConfig 结构体，添加以下字段

type AIConfig struct {
	// ... 现有字段（保持不变）
	ChatModel   ChatModelConfig   `toml:"chatModel"`
	Embedding   EmbeddingConfig   `toml:"embedding"`
	
	// ========== 新增：微服务配置 ==========
	Microservice MicroserviceConfig `toml:"microservice"`
}
```

**添加新的配置结构体**

在 `config.go` 文件末尾添加以下代码：

```go
// ========== 微服务配置结构 ==========

// MicroserviceConfig 微服务总配置
//
// 设计原理：
// 1. 每个功能独立配置（input_prediction、polish、digest）
// 2. 支持全局开关（Enabled）
// 3. 支持调用日志开关（LogCalls）
type MicroserviceConfig struct {
	Enabled bool `toml:"enabled"` // 是否启用微服务（总开关）
	LogCalls bool `toml:"log_calls"` // 是否记录调用日志（开发环境true，生产环境可选）
	
	// 各功能独立配置
	InputPrediction ServiceModelConfig `toml:"input_prediction"` // 智能输入预测
	Polish          ServiceModelConfig `toml:"polish"`           // 文本润色
	Digest          ServiceModelConfig `toml:"digest"`           // 消息摘要
}

// ServiceModelConfig 单个微服务的模型配置
//
// 设计原理：
// 1. 每个功能可以使用不同的模型
// 2. 支持不同的 Provider（ark、openai、deepseek）
// 3. 支持功能特定的参数（如 ContextMessages）
//
// 配置示例：
//   [aiConfig.microservice.input_prediction]
//   provider = "ark"
//   model = "doubao-lite-8k"
//   api_key = "${DOUBAO_API_KEY}"
//   temperature = 0.7
//   max_tokens = 100
type ServiceModelConfig struct {
	// ========== LLM 基础配置 ==========
	Provider        string  `toml:"provider"`         // 提供商：ark/openai/deepseek
	Model           string  `toml:"model"`            // 模型名称
	APIKey          string  `toml:"api_key"`          // API Key（支持环境变量）
	BaseURL         string  `toml:"base_url"`         // API Base URL
	Temperature     float64 `toml:"temperature"`      // 温度（0-1）
	MaxTokens       int     `toml:"max_tokens"`       // 最大 Token 数
	TimeoutSeconds  int     `toml:"timeout_seconds"`  // 超时时间（秒）
	
	// ========== 功能特定配置 ==========
	// 以下参数为可选，不同功能使用不同参数
	
	// 智能输入预测专用
	ContextMessages  int `toml:"context_messages"`   // 上下文消息数（默认10）
	DebounceMsint    int `toml:"debounce_ms"`        // 防抖延迟（毫秒，默认500）
	MaxInputChars    int `toml:"max_input_chars"`    // 最大输入字符（默认500）
	CacheTTLSeconds  int `toml:"cache_ttl_seconds"`  // 缓存TTL（秒）
	
	// 润色专用
	MaxOptions       int `toml:"max_options"`        // 最多返回选项数（默认3）
	
	// 摘要专用
	MaxMessages      int `toml:"max_messages"`       // 最多处理消息数（默认200）
}
```

### 代码说明

#### 5.1.1.1 设计要点

##### 1. 配置分层

```
MicroserviceConfig (总配置)
├─ Enabled (全局开关)
├─ LogCalls (日志开关)
├─ InputPrediction (智能输入配置)
│  ├─ Provider, Model, APIKey ... (通用配置)
│  └─ ContextMessages, DebounceMs ... (功能特定配置)
├─ Polish (润色配置)
│  ├─ Provider, Model, APIKey ... (通用配置)
│  └─ MaxOptions ... (功能特定配置)
└─ Digest (摘要配置)
   ├─ Provider, Model, APIKey ... (通用配置)
   └─ MaxMessages ... (功能特定配置)
```

##### 2. 环境变量支持

```toml
# config.toml 中可以使用环境变量
api_key = "${DOUBAO_API_KEY}"

# 运行时会自动替换
export DOUBAO_API_KEY="your-api-key"
```

##### 3. 可选参数

```go
// 某些参数只在特定功能中使用
type ServiceModelConfig struct {
    // 通用参数（所有功能都需要）
    Provider string
    Model string
    
    // 可选参数（只在特定功能中使用）
    ContextMessages int  // 只用于 input_prediction
    MaxOptions int       // 只用于 polish
    MaxMessages int      // 只用于 digest
}
```

---

### 5.1.2 修改 config_local.toml

#### 文件路径
```
configs/config_local.toml
```

#### 添加内容

在现有配置文件的 `[aiConfig]` 段后面添加以下内容：

```toml
# ========== AI 微服务配置 ==========
[aiConfig.microservice]
enabled = true        # 是否启用微服务功能
log_calls = true      # 是否记录调用日志（开发环境建议true）

  # ---------- 智能输入预测配置 ----------
  [aiConfig.microservice.input_prediction]
  provider = "ark"                                        # 提供商（ark/openai）
  model = "doubao-lite-8k"                               # 模型名称
  api_key = "${DOUBAO_API_KEY}"                          # API Key（从环境变量读取）
  base_url = "https://ark.cn-beijing.volces.com/api/v3"  # API 地址
  temperature = 0.7                                      # 温度（0-1，越高越随机）
  max_tokens = 100                                       # 最大生成 Token 数
  timeout_seconds = 5                                    # 超时时间（秒）
  
  # 功能特定配置
  context_messages = 10      # 上下文消息数
  debounce_ms = 500          # 防抖延迟（毫秒）
  max_input_chars = 500      # 最大输入字符
  cache_ttl_seconds = 300    # 缓存 TTL（秒，5分钟）

  # ---------- 文本润色配置 ----------
  [aiConfig.microservice.polish]
  provider = "ark"
  model = "doubao-lite-8k"
  api_key = "${DOUBAO_API_KEY}"
  base_url = "https://ark.cn-beijing.volces.com/api/v3"
  temperature = 0.8          # 润色需要更高的温度（更有创意）
  max_tokens = 200           # 润色需要更多 Token
  timeout_seconds = 10
  
  # 功能特定配置
  max_options = 3            # 最多返回3个润色选项
  cache_ttl_seconds = 1800   # 缓存 TTL（秒，30分钟）

  # ---------- 消息摘要配置 ----------
  [aiConfig.microservice.digest]
  provider = "ark"
  model = "doubao-pro-32k"   # 摘要使用更强的模型（支持更长上下文）
  api_key = "${DOUBAO_API_KEY}"
  base_url = "https://ark.cn-beijing.volces.com/api/v3"
  temperature = 0.5          # 摘要需要更低的温度（更准确）
  max_tokens = 500           # 摘要需要更多 Token
  timeout_seconds = 15
  
  # 功能特定配置
  max_messages = 200         # 最多处理200条消息
  cache_ttl_seconds = 600    # 缓存 TTL（秒，10分钟）
```

### 配置说明

#### 5.1.2.1 环境变量设置

在启动应用前，需要设置环境变量：

```bash
# Linux/Mac
export DOUBAO_API_KEY="your-doubao-api-key"

# Windows (PowerShell)
$env:DOUBAO_API_KEY = "your-doubao-api-key"

# Windows (CMD)
set DOUBAO_API_KEY=your-doubao-api-key
```

#### 5.1.2.2 参数调优建议

| 功能 | 温度 | MaxTokens | 模型选择 | 理由 |
|------|------|-----------|----------|------|
| **智能输入** | 0.7 | 100 | doubao-lite-8k | 需要平衡准确性和多样性 |
| **润色** | 0.8 | 200 | doubao-lite-8k | 需要更有创意的表达 |
| **摘要** | 0.5 | 500 | doubao-pro-32k | 需要准确性，支持长上下文 |

#### 5.1.2.3 成本优化

```toml
# 开发环境：使用小模型降低成本
[aiConfig.microservice.input_prediction]
model = "doubao-lite-8k"  # ¥0.0003/1K tokens

# 生产环境：根据实际需求选择
[aiConfig.microservice.digest]
model = "doubao-pro-32k"  # ¥0.003/1K tokens（10倍价格，但质量更好）
```

---

## 5.2 路由注册

### 5.2.1 修改 https_server.go

#### 文件路径
```
api/http/https_server.go
```

#### 修改位置

找到路由注册部分（通常在 `InitRouters` 函数中），在现有AI路由之后添加微服务路由。

#### 完整添加代码

```go
// 在 InitRouters 函数中，找到 AI 路由注册部分，添加以下代码

func InitRouters(router *gin.Engine, conf *config.Config) {
	// ... 现有代码（保持不变）
	
	// ===== 现有 AI Assistant 路由 =====
	// authed.GET("/ai/assistant/sessions", aiAssistantH.ListSessions)
	// authed.POST("/ai/assistant/chat", aiAssistantH.Chat)
	// ... 其他现有路由
	
	// ========== 新增：AI 微服务路由 ==========
	//
	// 设计要点：
	// 1. 检查配置是否启用
	// 2. 初始化所有依赖（按顺序）
	// 3. 注册 HTTP 和 WebSocket 路由
	if conf.AIConfig.Microservice.Enabled {
		zlog.Info("initializing AI microservice...")
		
		// ========== Step 1: 初始化轻量模型 ==========
		//
		// 设计要点：
		// - 使用专用的轻量模型（区别于 AI Assistant）
		// - 成本优先（使用小模型）
		microChatModel, err := aiLLM.NewLightweightChatModel(ctx, conf)
		if err != nil {
			zlog.Fatal("failed to init lightweight chat model",
				zap.Error(err))
		}
		zlog.Info("lightweight chat model initialized",
			zap.String("provider", conf.AIConfig.Microservice.InputPrediction.Provider),
			zap.String("model", conf.AIConfig.Microservice.InputPrediction.Model))
		
		// ========== Step 2: 初始化 Redis Cache ==========
		//
		// 设计要点：
		// - 复用现有的 Redis 连接
		// - 如果 Redis 不可用，传 nil（禁用缓存）
		var redisCache pipeline.CacheInterface
		if initial.RedisClient != nil {
			redisCache = NewRedisCache(initial.RedisClient)
			zlog.Info("redis cache initialized for microservice")
		} else {
			zlog.Warn("redis not available, cache disabled for microservice")
		}
		
		// ========== Step 3: 初始化 MicroservicePipeline ==========
		//
		// 设计要点：
		// - Pipeline 负责插件管理和 LLM 调用
		// - 自动注册默认插件（input_prediction、polish、digest）
		microPipeline := aiPipeline.NewMicroservicePipeline(
			microChatModel,
			redisCache,
		)
		zlog.Info("microservice pipeline initialized")
		
		// ========== Step 4: 初始化 Service ==========
		//
		// 设计要点：
		// - Service 负责业务逻辑编排
		// - 依赖注入 Pipeline
		microService := aiService.NewAIMicroserviceService(microPipeline)
		zlog.Info("microservice service initialized")
		
		// ========== Step 5: 初始化 Handlers ==========
		//
		// 设计要点：
		// - HTTP Handler 处理 Polish、Digest
		// - WebSocket Handler 处理 PredictStream
		microHTTPHandler := aiHTTP.NewMicroserviceHandler(microService)
		microWSHandler := aiWS.NewMicroserviceWSHandler(microService)
		zlog.Info("microservice handlers initialized")
		
		// ========== Step 6: 注册路由 ==========
		//
		// 设计要点：
		// - 所有路由都在 /ai/microservice 下
		// - 使用 authed 中间件（需要 JWT 认证）
		// - WebSocket 使用 GET 方法
		aiMicroGroup := authed.Group("/ai/microservice")
		{
			// HTTP 接口（非流式）
			aiMicroGroup.POST("/predict", microHTTPHandler.Predict)  // 智能输入预测（测试用）
			aiMicroGroup.POST("/polish", microHTTPHandler.Polish)    // 文本润色
			aiMicroGroup.POST("/digest", microHTTPHandler.Digest)    // 消息摘要
			
			// WebSocket 接口（流式）
			aiMicroGroup.GET("/input/ws", microWSHandler.InputPrediction) // 智能输入预测（流式）
		}
		
		zlog.Info("AI microservice routes registered",
			zap.String("base_path", "/ai/microservice"))
	} else {
		zlog.Info("AI microservice is disabled in config")
	}
	
	// ... 其他路由
}
```

#### 需要新增的辅助函数

在 `https_server.go` 文件中添加 Redis Cache 适配器：

```go
// ========== Redis Cache 适配器 ==========

// RedisCache Redis 缓存实现（实现 pipeline.CacheInterface）
//
// 设计原理：
// - 适配器模式：将 Redis Client 适配为 Pipeline 需要的接口
// - 简单封装：只实现 Get、Set、Delete 三个基本方法
type RedisCache struct {
	client *redis.Client
}

// NewRedisCache 创建 Redis 缓存适配器
func NewRedisCache(client *redis.Client) *RedisCache {
	return &RedisCache{client: client}
}

// Get 获取缓存
func (c *RedisCache) Get(ctx context.Context, key string) (string, error) {
	val, err := c.client.Get(ctx, key).Result()
	if err == redis.Nil {
		return "", fmt.Errorf("key not found")
	}
	return val, err
}

// Set 设置缓存
func (c *RedisCache) Set(ctx context.Context, key string, value string, ttl time.Duration) error {
	return c.client.Set(ctx, key, value, ttl).Err()
}

// Delete 删除缓存
func (c *RedisCache) Delete(ctx context.Context, key string) error {
	return c.client.Del(ctx, key).Err()
}
```

### 代码说明

#### 5.2.1.1 初始化顺序

```
1. LightweightChatModel (LLM 模型)
   ↓
2. RedisCache (缓存)
   ↓
3. MicroservicePipeline (Pipeline)
   ↓
4. AIMicroserviceService (Service)
   ↓
5. Handlers (HTTP + WebSocket)
   ↓
6. 路由注册
```

**为什么这样顺序？**
- ✅ 依赖关系清晰（底层 → 上层）
- ✅ 初始化失败时能快速定位
- ✅ 日志输出有序

#### 5.2.1.2 错误处理策略

```go
// LLM 初始化失败 → 致命错误（无法提供服务）
if err != nil {
    zlog.Fatal("failed to init lightweight chat model", zap.Error(err))
}

// Redis 初始化失败 → 警告（禁用缓存，但可以继续）
if initial.RedisClient != nil {
    redisCache = NewRedisCache(initial.RedisClient)
} else {
    zlog.Warn("redis not available, cache disabled")
    redisCache = nil // Pipeline 会处理 nil 的情况
}
```

#### 5.2.1.3 路由设计

| 路径 | 方法 | 功能 | 类型 |
|------|------|------|------|
| `/ai/microservice/predict` | POST | 智能输入预测 | HTTP（测试用） |
| `/ai/microservice/polish` | POST | 文本润色 | HTTP |
| `/ai/microservice/digest` | POST | 消息摘要 | HTTP |
| `/ai/microservice/input/ws` | GET | 智能输入预测 | WebSocket（推荐） |

---

## 5.3 WebSocket 中间件（可选）

### 5.3.1 创建 WebSocket 认证中间件

#### 文件路径
```
internal/middleware/ws_auth.go
```

#### 完整代码

```go
package middleware

import (
	"net/http"
	"strings"

	"OmniLink/internal/middleware/jwt"
	"OmniLink/pkg/zlog"

	"github.com/gin-gonic/gin"
	"go.uber.org/zap"
)

// WSAuthMiddleware WebSocket 连接时的 JWT 验证
//
// 设计原理：
// 1. WebSocket 无法在握手时设置自定义 Header
// 2. 支持三种认证方式（按优先级）：
//    - URL Query 参数：?token=xxx（推荐）
//    - HTTP Header：Authorization: Bearer xxx
//    - Cookie：auth_token=xxx
// 3. 验证成功后将 uuid 存入 Context
//
// 使用场景：
// - WebSocket 路由需要 JWT 认证
// - 兼容多种客户端实现
//
// 使用方式：
//   aiMicroGroup.GET("/input/ws", WSAuthMiddleware(), microWSHandler.InputPrediction)
func WSAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		var token string

		// ========== Step 1: 从 URL Query 获取 Token ==========
		//
		// 优先级最高，因为 WebSocket 客户端最容易使用
		// 示例：ws://localhost:8080/ai/microservice/input/ws?token=xxx
		token = c.Query("token")

		// ========== Step 2: 从 HTTP Header 获取 Token ==========
		//
		// 如果 Query 没有，尝试从 Header 获取
		// 示例：Authorization: Bearer xxx
		if token == "" {
			authHeader := c.GetHeader("Authorization")
			if strings.HasPrefix(authHeader, "Bearer ") {
				token = strings.TrimPrefix(authHeader, "Bearer ")
			}
		}

		// ========== Step 3: 从 Cookie 获取 Token ==========
		//
		// 如果前两者都没有，尝试从 Cookie 获取
		if token == "" {
			cookie, err := c.Cookie("auth_token")
			if err == nil {
				token = cookie
			}
		}

		// ========== Step 4: 验证 Token ==========
		if token == "" {
			zlog.Warn("websocket auth failed: missing token",
				zap.String("remote_addr", c.Request.RemoteAddr))
			c.JSON(http.StatusUnauthorized, gin.H{"error": "missing token"})
			c.Abort()
			return
		}

		// 解析和验证 Token
		claims, err := jwt.ParseToken(token)
		if err != nil {
			zlog.Warn("websocket auth failed: invalid token",
				zap.Error(err),
				zap.String("remote_addr", c.Request.RemoteAddr))
			c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid token"})
			c.Abort()
			return
		}

		// ========== Step 5: 存储用户信息 ==========
		//
		// 将 uuid 存入 Context，供 Handler 使用
		c.Set("uuid", claims.UUID)

		zlog.Info("websocket auth success",
			zap.String("uuid", claims.UUID),
			zap.String("remote_addr", c.Request.RemoteAddr))

		c.Next()
	}
}
```

### 代码说明

#### 5.3.1.1 认证方式对比

| 方式 | 优点 | 缺点 | 使用场景 |
|------|------|------|----------|
| **URL Query** | 简单，浏览器支持 | Token 暴露在 URL | WebSocket 推荐 |
| **HTTP Header** | 安全，不暴露 | 需要 JS 设置 | AJAX 请求推荐 |
| **Cookie** | 自动发送 | 跨域问题 | 同域场景 |

#### 5.3.1.2 使用方式

```go
// 方式1：全局应用（所有 WebSocket 都需要认证）
authed := router.Group("/api", WSAuthMiddleware())

// 方式2：单个路由应用（推荐）
router.GET("/ws", WSAuthMiddleware(), handler.WebSocket)

// 方式3：不使用中间件（在 Handler 内部认证）
router.GET("/ws", handler.WebSocket) // Handler 内部自己处理认证
```

---

## 第五部分总结

### 已完成的内容

1. ✅ **配置文件修改**
   - config.go 添加配置结构（~80行）
   - config_local.toml 添加配置示例（~60行）

2. ✅ **路由注册**
   - https_server.go 初始化和路由注册（~100行）
   - Redis Cache 适配器（~30行）

3. ✅ **WebSocket 中间件**
   - ws_auth.go JWT 认证中间件（~80行）

### 代码统计

- **总行数**: ~350行（含注释）
- **配置行数**: ~140行
- **代码行数**: ~210行

### 核心特性

- ✅ **配置分层**: 总开关 + 功能配置
- ✅ **环境变量支持**: 敏感信息不硬编码
- ✅ **依赖注入**: 清晰的初始化顺序
- ✅ **多种认证方式**: Query/Header/Cookie
- ✅ **优雅降级**: Redis 不可用时禁用缓存

---

## 5.4 验证配置

### 5.4.1 启动检查

修改后，启动应用应该看到以下日志：

```
INFO  initializing AI microservice...
INFO  lightweight chat model initialized  provider=ark model=doubao-lite-8k
INFO  redis cache initialized for microservice
INFO  microservice pipeline initialized
INFO  microservice service initialized
INFO  microservice handlers initialized
INFO  AI microservice routes registered  base_path=/ai/microservice
```

### 5.4.2 路由验证

启动后，访问以下URL验证路由是否注册成功：

```bash
# 查看所有路由（Gin Debug 模式）
# 应该能看到：
# [GIN-debug] POST   /ai/microservice/predict
# [GIN-debug] POST   /ai/microservice/polish
# [GIN-debug] POST   /ai/microservice/digest
# [GIN-debug] GET    /ai/microservice/input/ws
```

### 5.4.3 配置测试

```bash
# 测试环境变量
echo $DOUBAO_API_KEY  # 应该输出你的 API Key

# 测试配置加载
# 启动应用，检查日志中是否有配置错误
```

---

## 下一步

### 已完成（1-5部分）

- ✅ Part 1: 插件系统（700行）
- ✅ Part 2: Pipeline层（410行）
- ✅ Part 3: Service层（500行）
- ✅ Part 4: Handler层（400行）
- ✅ Part 5: 配置和路由（350行）

**后端核心代码已完成**: ~2360行

### 待完成（6-9部分）

- 📝 Part 6: 数据库迁移（170行）
- 📝 Part 7: 前端实现（900行）
- 📝 Part 8: 测试指南
- 📝 Part 9: 部署清单

---

## 🎉 重要里程碑

**恭喜！后端核心代码已全部完成！**

现在您可以：
1. ✅ 创建数据库表（Part 6）
2. ✅ 启动后端服务
3. ✅ 使用 Postman 测试 API
4. ✅ 使用 wscat 测试 WebSocket

---

## 测试建议

在继续前端开发之前，建议先测试后端：

```bash
# 1. 设置环境变量
export DOUBAO_API_KEY="your-api-key"

# 2. 启动服务
go run cmd/OmniLink/main.go

# 3. 测试 HTTP 接口（Postman）
POST http://localhost:8080/ai/microservice/polish
Authorization: Bearer <your-jwt-token>
{
  "text": "给我发一下那个文件"
}

# 4. 测试 WebSocket（wscat）
wscat -c "ws://localhost:8080/ai/microservice/input/ws?token=<your-jwt-token>"
> {"action":"predict","data":{"input":"今天天气真不错"}}
```

---

## ❓ 下一步选择

您现在可以：

### 选项1：继续Part 6（数据库迁移）
- 创建SQL迁移脚本
- 插入初始配置数据
- 快速完成（~100行）

### 选项2：跳到Part 7（前端实现）
- 创建Vue组件
- API服务封装
- WebSocket管理器
- 代码量最大（~900行）

### 选项3：先测试后端
- 创建所有必要的代码文件
- 运行测试
- 确保后端正常工作

---

请告诉我您的选择！
