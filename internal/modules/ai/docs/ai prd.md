...
模块五：动态上下文画布 (Dynamic Context Canvas)
痛点：传统 IM 只有“线性信息流”。聊到代码要切 IDE，聊到旅游要切地图，聊到数据要切 Excel。 AI Native 方案：聊天窗口不仅仅是气泡，是一个即时生成的 App。

Generative UI (生成式界面)：

场景：当群里在讨论“周末去哪吃火锅”，AI 不仅总结，而是直接在聊天流中渲染出一个 投票卡片 或 地图卡片，上面标注了大家的距离。

实现：AI 识别意图 -> 生成一段 JSON -> 前端（Flutter/React/QT）动态渲染组件。

协作白板 (Infinite Whiteboard)：

场景：工作模式下，输入 /brainstorm，聊天背景变暗，变为无限白板。用户的每条文字消息自动变成白板上的便利贴，AI 自动负责将相似观点的便利贴聚类（Clustering）。

模块六：AI 社交润滑剂 (The Social Lubricant)
痛点：冷场、不知道怎么回、情绪误解、群聊由于话题不同步导致的混乱。 AI Native 方案：AI 作为社交中介，负责“预处理”和“后渲染”。

情绪转译 (Sentiment Translator)：

功能：给社恐或直男用户的“高情商外挂”。

操作：用户输入“这方案太烂了，重做”，选择【委婉模式】发送。对方看到的是：“我觉得这个方案还有提升空间，建议我们在以下几个方面调整一下...”

话题分流 (Topic Threading)：

功能：大群里经常出现两个人聊技术，三个人聊八卦。AI 自动检测语义，建议：“检测到你们在深入讨论 Golang，是否创建一个临时的‘技术角’子频道？”

气氛组 Agent：

功能：娱乐场景下，当群里检测到大家都在发“哈哈哈哈”，AI 自动生成一个符合当前梗的表情包（Meme Generator），或者自动播放一段欢庆的背景音效。

模块七：数字分身“离线模式” (Offline Avatar Mode)
痛点：人不可能 24 小时在线，但消息是 24 小时的。 AI Native 方案：当你下线时，你的 Agent 接管身体。

有限授权代理：

设定：你可以开启“工作代理”或“闲聊代理”。

场景：你正在睡觉，老板问：“上次那个文档在哪？”你的 Agent 检索你的 RAG 权限，自动回复：“主人的历史记录显示，该文档于周三发在了群文件里，链接是 [URL]（由 AI 自动回复）”。

标记：必须明确标注这是 [AI 代理回复]，避免伦理问题。
...
E) 基于 Eino 的编排建议（不手搓）
- 目标：把“检索 + 生成”拆成可替换的 Node/Chain，并且让 HTTP Handler 只做参数解析与鉴权。
- 推荐形态：在 application 或 pipeline 层新增 retrieve/answer pipeline，内部使用 Eino compose 编排（类似你现在 ingest_pipeline 的 buildGraph 思路）。
- 节点建议（顺序）：
  1) Normalize/Validate：清洗 query/topK/scope，构造 filter expr（必须包含 tenant_user_id）。
  2) EmbedQuery：调用 Eino embedding.Embedder 生成 query 向量（与入库同 provider/model/dim）。
  3) Retrieve：调用 repository.VectorStore.Search(vector, topK, expr)。
  4) PostProcess：去重/阈值过滤/排序/按 source 合并/截断。
  5) BuildContext：把命中的 chunks 格式化成上下文文本（并保留 citations）。
  6) Answer：调用 Eino 的 ChatModel（或 LLM 组件）生成回答；响应里必须返回 citations。

验收标准（检索闭环）：
- 提问一个你确定在历史消息出现过的信息（人名/日期/承诺），能返回包含引用的答案。

4.4.3 全局 AI 个人助手对话闭环（Chat + RAG + Memory + Streaming）
目标：让用户能在一个固定入口里与“全局助手”多轮对话，并且每一轮可自动触发 RAG（全域但权限内）。

A) 最小对话入口（HTTP）
- 建议路由：
  - /ai/assistant/chat：非流式（返回完整回答 + citations）
  - /ai/assistant/chat/stream：流式（建议 SSE 起步，稳定后再扩展 WebSocket）
- 鉴权：与现有 authed 组一致，从 JWT 中取 uuid 作为 tenant_user_id。

B) 会话与记忆（Memory）
- 目标：同一个 session_id 下多轮对话可复盘，上下文可控（只取最近 N 轮）。
- 建议存储（避免与 IM chat 的表强耦合）：新增 AI 助手专用会话/消息表，例如：
  - ai_assistant_session：tenant_user_id、title、status、created_at/updated_at
  - ai_assistant_message：session_id、role(user/assistant/system)、content、citations_json、created_at
- 记忆策略（第一版即可）：加载最近 N 条 message（或最近 N 轮 user+assistant），拼进 prompt 的 history。

C) RAG 的作用域与权限
- 默认 scope：global（即该用户的 global KB，检索条件必须包含 tenant_user_id + kb_id）。
- 可选 scope：按私聊/群聊收窄（通过 source_type/source_key 过滤）。
- 约束：scope 只是“检索过滤”，不改变底层存储结构。

D) 基于 Eino 的对话编排建议（Assistant Chain）
- 推荐用 Eino compose 把 assistant 的一次对话编排成一个 Runnable（每次请求调用一次），节点建议：
  1) LoadMemory：按 tenant_user_id + session_id 取历史消息（最近 N 轮）。
  2) Retrieve：复用 4.4.2 的 Retrieve/Answer pipeline 中的 Retrieve/PostProcess/BuildContext。
  3) BuildPrompt：system persona + memory + retrieved context + user question。
  4) ChatModel：生成回答（stream 版本必须支持 token 流式输出）。
  5) Persist：落库本轮 user/assistant message，并记录 citations。

E) 验收标准（对话闭环）
- 非流式：连续发 3 轮问答，能保持上下文不丢失；每轮都有 citations（命中为空则 citations 为空但接口不报错）。
- 流式：能持续输出 token，并在结束时返回完整 citations（可放在最后一个事件/最终响应体中）。
...
开发计划（按“先可用、再增强”的顺序，但每一步都能独立验收）

阶段 1：先把“召回（Retrieve）”做成可观测的闭环（不接 Chat 也行）

- 目标 ：给你一个稳定的 /ai/rag/query 调试接口：输入 question → 返回 topK chunks（带 chunk_id/source_type/source_key/score/content）
- Eino 使用方式 ：做一个 RetrievePipeline（compose graph/chain），节点顺序固定为：
  - Normalize/Validate（tenant、topK、过滤条件）
  - Query Embedding（直接用 embedding.Embedder ，你已有 mock，后面可替换真实 provider）
  - Vector Search（调用 VectorStore.Search(vector, topK, expr) ，expr 至少包含 tenant_user_id = 当前用户 AND kb_id=global ）
  - PostProcess（去重、阈值过滤、按 score 排序、可选“同 source 合并/按时间偏置”）
- 验收 ：随便问一句你确定在历史消息里出现过的内容，接口能返回命中 chunks，且 tenant_user_id 隔离正确
阶段 2：把“LLM 生成回答”接到召回后面（RAG Answer）

- 目标 ： /ai/rag/answer ：输入 question → 输出 answer + citations（引用 chunk_id 列表）
- Eino 使用方式 ：用 Eino 的 Prompt/Chain 来拼装：
  - Context Builder：把命中的 chunks 格式化成可控长度的上下文（包含引用标记）
  - Prompt Template：system + user question + context
  - ChatModel：接入一个 Eino 的 chat model 组件（需要你选定厂商并引入对应 Eino 扩展依赖；你项目目前还没接入 LLM provider）
- 验收 ：回答文本包含引用标记（比如 [chunk:123] ），并能在响应里返回 citations 数组
阶段 3：实现“全局 AI 个人助手 Chat”（带会话与流式）

- 目标 ： /ai/assistant/chat （非流式）+ /ai/assistant/chat/stream （流式），支持：
  - 同一用户多轮对话（有 session_id）
  - 每轮自动 RAG（默认走 global KB）
  - 返回 assistant 消息 + citations
- 存储建议（我替你做决策，避免纠结） ：独立存到 AI 模块的会话/消息表，不复用 IM chat 的表，避免把“AI 对话记忆”和“IM 会话”耦死、权限/展示逻辑混在一起
- Eino 使用方式 ：AssistantChain（compose chain）固定结构：
  - LoadMemory（按 session_id 取最近 N 轮历史）
  - Retrieve（复用阶段 1 的 RetrievePipeline）
  - BuildPrompt（system persona + memory + retrieved context + user message）
  - ChatModel（支持 streaming）
  - Persist（落库：user/assistant message、citations、耗时等）
- 流式形态建议 ：先用 SSE（实现最简单、和 gin 兼容），WebSocket 留到后面；你当前 IM 的 WS 是给消息实时用的（见现有 ws handler），AI 流式更适合 SSE 起步
阶段 4：把“质量与成本控制”补齐（上线前必须有，但不影响前面跑通）

- 召回质量 ：score 阈值、topK 动态、同 source 去重、按时间偏置（对聊天记录尤其重要）
- 上下文控制 ：最大 chunks 数、最大字符/Token（避免把 LLM 窗口打爆）
- 观测与回放 ：每次请求记录 query_id、topK、命中 chunk_id、耗时、LLM 用量、错误原因（方便你调参）
- 兜底策略 ：RAG 命中为空时，直接走“纯聊天回答”或提示“需要回填/未命中”，别返回空白
落地顺序（你照着做不容易跑偏）

- 先补齐 LLM provider（只要能用 Eino ChatModel 调通一次；mock 不解决“chat 功能”）
- 做 /ai/rag/query （只做检索，不接 LLM），确保召回正确
- 做 /ai/rag/answer （把检索结果喂给 LLM），确保 citations 链路打通
- 做 /ai/assistant/chat(+stream) （加 session/memory），确保多轮稳定
- 最后再做增强：rerank、query rewrite、MCP/工具调用（这些都挂在“LLM Answer”阶段后面，不要提前做）